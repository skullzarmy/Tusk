 ...
 *
 * Error (if it exists) is guaranteed to be Mastodon error-formatted.
 * String (if it exists) is the bearer token received from Mastodon.
 */
exports.getBearerToken = function (consumer_key, consumer_secret, cb) {
  // use OAuth 2 for app-only auth (Mastodon requires this)
  ...
  request.post({
    ...
    if (err) {
      var error = exports.makeMastodonError(err.toString());
      exports.attachBodyInfoToError(error, body);
      return cb(error, body, res);
    }

    if ( !body ) {
      var error = exports.makeMastodonError('Not valid reply from Mastodon upon obtaining bearer token');
      exports.attachBodyInfoToError(error, body);
      return cb(error, body, res);
    }

    if (body.token_type !== 'bearer') {
      var error = exports.makeMastodonError('Unexpected reply from Mastodon upon obtaining bearer token');
      exports.attachBodyInfoToError(error, body);
      return cb(error, body, res);
    }
